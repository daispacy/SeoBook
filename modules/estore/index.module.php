...
$productCategoryItems = $productCategories->getObjects(1,"`status` = '1' and parent_id = 0",array('position' => 'ASC'),200);
if($productCategoryItems) {	
	$template->assign('listCateProducts',$productCategoryItems );
}

# Get the hot product list
$products = new Products($storeId);
$productHot = $products->getObjects(1,"status = 1 AND home = 1",array('created'=>'DESC'),16);
$template->assign('productHot',$productHot);

$productNewest = $products->getObjects(1,"status = 1",array('created'=>'DESC'),16);
$template->assign('productNewest',$productNewest);

# Get the new news list
include_once(ROOT_PATH.'classes/dao/projects.class.php');
$projects= new Projects($storeId);
$projectItems = $projects->getObjects(1,"`status` = '1'",array('date_created' => 'DESC'),8);
if($projectItems)$template->assign('projectItems',$projectItems);
# Get the product list
/*
$estore->setProperty('products_per_page', 5);
...
$aboutUs = $statics->getObjectFromSlug('gioi-thieu'); 
