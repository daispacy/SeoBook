	...
	if($request->element('doo') == 'submit') {
		...
		if($validate['invalid']) {	# data input is not in valid form
			...
			if(!$validate['invalid']) {
                ...
                $properties['link_fb'] = $request->element('link_fb');
                $properties['link_gg'] = $request->element('link_gg');
				# Custom fields
...
function validateData($request) {
	...
	if($error['INPUT']['site_name']['error'] || $error['INPUT']['keywords']['error'] || $error['INPUT']['site_description']['error'] || $error['INPUT']['currency']['error']) {
		$error['invalid'] = 1;
		return $error;
	}
	$error['invalid'] = 0;
