...
if(!$staticInfo) {	
	...
	if($_POST && $request->element('doo') == 'submit') { # if form is submitted
		...
		$fieldList = $fields->getObjects(1,"`status`='1' AND `module`='project'",array('position' => 'ASC'));
		...
		if($validate['invalid']) {	# data input is not in valid form
			...
			if(!$validate['invalid']) {
				...
				if($staticInfo) {			
					...
					header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=project&mod=edit&lang=$lang&id=$id&rcode=7");
				}
			}
		}
