...
$url = '/'.ADMIN_SCRIPT."?op=manage&act=product&mod=list&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=%d&dk=$dk";
...
# Link
$link = '/'.ADMIN_SCRIPT."?op=manage&act=product&mod=list&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&pg=$page&dk=$dk";
...
if($_POST) {
	switch($do) {
		...
		case 'setseller':
			...
			$id = $request->element('id');
			if($id) {
				$products->changeSeller($id,S_ENABLED);
				$result_code = 7;
				# Operation tracking
				$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf("Set sản phẩm %s thành sản phẩm bán chạy",$products->getNameFromId($id)),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			} 
			break;
		case 'deleteseller':
			$userInfo->checkPermission('product','edit');
			$id = $request->element('id');
			if($id) {
				$products->changeSeller($id,S_DISABLED);
				$result_code = 7;
				# Operation tracking
				$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf("Hủy sản phẩm %s thành sản phẩm bán chạy",$products->getNameFromId($id)),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			} 
		break;
		case 'setspecial':
			$userInfo->checkPermission('product','edit');
			$id = $request->element('id');
			if($id) {
				$products->changeSpecial($id,S_ENABLED);
				$result_code = 7;
				# Operation tracking
				$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf("Set sản phẩm %s thành sản phẩm đặc biệt",$products->getNameFromId($id)),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			} 
			break;
		case 'deletespecial':
			$userInfo->checkPermission('product','edit');
			$id = $request->element('id');
			if($id) {
				$products->changeSpecial($id,S_DISABLED);
				$result_code = 7;
				# Operation tracking
				$trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf("Hủy sản phẩm %s thành sản phẩm đặc biệt",$products->getNameFromId($id)),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
			} 
		break;
		case 'deletehome':
			$userInfo->checkPermission('product','edit');
		...
		case 'changeposition':
			...
			$market_prices = $request->element('market_prices');
			if($positions && $ids) {
			...
			if($prices && $ids) {
				foreach ($ids as $id) {
					$products->changePrice($id,$prices[$id]);
				}
				$result_code = 4;
				# Operation tracking
			...
			if($market_prices && $ids) {
				foreach ($ids as $id) {							
						if($products->changeMarketPrice($id,$market_prices[$id]))
						$result_code = 4;	
						else $error_code = 11;
				}
		...
		case 'cleantrash':
			...
			break;		
		case 'cancel':		
			header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=product&mod=list&lang=$lang&ecode=7&pId=$pId&dk=$dk");
			exit;
			break;
	}
	header('location:'.'/'.ADMIN_SCRIPT."?op=manage&act=product&mod=list&doo=$do&kw=$kw&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pg=$page&ecode=$error_code&rcode=$result_code&pId=$pId&dk=$dk");
} else {
