...
include_once(ROOT_PATH."classes/dao/sizeinfo.class.php");

class Sizes extends Model {
	var $table;
	var $_db;

	function Sizes($database = '') {
		if(!$database) {
			global $db;
			$this->_db = $db;
		} else $this->_db = $database;
		$this->table = DB_PREFIX."size";
	}
...
# Clean trash
	...
	function generateComboFromPid($pid) {
		
		$combo = '';
		$results = $this->select('*',"status = '1' and id in (select sid from dc_product_size where pid=$pid)");
		if($results) {
			foreach($results as $key => $result) {
				$combo .= "<option value='".$result['id']."'><strong>".$result['name']."</strong></option>";						
			}
		}
		return $combo;
	}
